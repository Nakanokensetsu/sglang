...
def resolve_mixed_kv_pool(kvcache):
    """Return the live ``UnifiedInt2HPKVPool`` behind ``kvcache``, or None.

    A hybrid GDN/Mamba model keeps its full-attention KV in
    ``HybridLinearKVPool.full_kv_pool``, so callers that only check the
    outer pool would silently conclude "mixed KV off" on exactly the models
    that wrap it. 2026-09-19 自前追加。
    """
    if kvcache is None:
        return None
    inner = getattr(kvcache, "full_kv_pool", None)
    if inner is not None:
        kvcache = inner
    if not isinstance(kvcache, UnifiedInt2HPKVPool):
        return None
    return kvcache if kvcache.mixed_kv_enabled() else None


def resolve_mixed_kv_pool_from_allocator(allocator):
    """``resolve_mixed_kv_pool`` starting from a token-to-KV allocator."""
    if allocator is None:
        return None
    getter = getattr(allocator, "get_kvcache", None)
    return resolve_mixed_kv_pool(getter() if getter is not None else None)
