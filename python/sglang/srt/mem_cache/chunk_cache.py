...
class ChunkCache(BasePrefixCache):
    ...
    def cache_finished_req(
        ...
        # PR #32129 retarget: the request-owned quant-page slack slots share
        # the row tail's atomic int2 quant page, and the int2 allocator folds
        # slot ids to whole pages with torch.unique -- so slack must ride in
        # the SAME free() call or the page is handed back twice. That rules out
        # free_kv_row()'s per-segment split; mixed-KV never runs under SWA, so
        # the split has nothing to do on this path anyway.
