...
class KVCacheConfigurator:
    ...
    def _mixed_kv_int2_enabled(self) -> bool:
        ...
        if self.mambaish_config is not None:
            # Mamba/GDN hybrids take the `_build_hybrid_linear_kv_pool` branch
            # above, so the unified pool is unreachable for them. Made explicit
            # here (2026-09-19) so the gate does not depend on branch order.
            return False
    ...
    def _build_token_to_kv_pool_allocator(
        ...
        *,
        ...
        if token_to_kv_pool_allocator is None:
            if resolve_mixed_kv_pool(token_to_kv_pool) is not None:
