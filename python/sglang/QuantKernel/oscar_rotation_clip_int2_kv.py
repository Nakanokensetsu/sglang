...
def _vectorized_elems_per_thread(dtype: torch.dtype) -> int:
    ...
    bf16/fp16 -> 8 (16 bits each), fp8 -> 16 (8 bits each). Other dtypes
    raise. Widened from upstream's bf16-only check for Escha-W2, which runs
    its model compute dtype as float16 (not bf16) -- see
    escha-w2-production-handover-2026-09-11 memory. This is purely a
    vectorized-load width computed from itemsize; the kernel itself has no
    bf16-specific numerics (grep confirms "bfloat16" appears nowhere else
    in this file), so fp16 is exactly as safe as bf16 here.
    """
    ...
    raise AssertionError(
        f"clip int2 kernel requires bf16, fp16, or fp8 input dtype, got {dtype}"
    )
