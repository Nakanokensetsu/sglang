...
PREFIXES = [" ".join(f"c{k}w{i}: {random.randint(0,999)}." for i in range(1500)) for k in range(CONC)]
stop = False; samples = []
def sampler():
    while not stop:
        try:
            o = subprocess.run(["nvidia-smi","--query-gpu=index,memory.used,utilization.gpu,power.draw,temperature.gpu",
...
def ask(k):
    body = json.dumps({"model": MODEL, "temperature": 0, "max_tokens": 200,
                       ...
                       "messages":[{"role":"system","content":PREFIXES[k]},
                                   {"role":"user","content":f"Reply with the number {k}."}]}).encode()
    ...
    try:
        with urllib.request.urlopen(req, timeout=1200) as r:
            d=json.load(r)
        u=d.get("usage",{})
        return {"k":k,"sec":round(time.time()-t0,2),"prompt":u.get("prompt_tokens",0),
...
print(f"  1周目 最遅={max(r['sec'] for r in res):.1f}s 最速={min(r['sec'] for r in res):.1f}s")
print(f"  2周目 最遅={max(r['sec'] for r in res2):.1f}s 最速={min(r['sec'] for r in res2):.1f}s")
for e in err[:3]: print("  ERR:", e)
print("  VRAM/util/power/temp サンプル(末尾5):")
for s in samples[-5:]: print("   ", s)
json.dump({"res":res,"res2":res2,"samples":samples}, open(sys.argv[1],"w"), indent=1)
