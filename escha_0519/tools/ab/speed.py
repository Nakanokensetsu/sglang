    ...
    with urllib.request.urlopen(req, timeout=1800) as r:
        ...
        for raw in r:
            ...
            if (
                ttft is None
                and d.get("choices")
                and d["choices"][0].get("delta", {}).get("content")
            ):
                ttft = time.time() - t0
            if d.get("usage"):
                usage = d["usage"]
...
# 1) 冷 prefill スループット
sec, _, u = call(
    [{"role": "system", "content": LONG}, {"role": "user", "content": "ok?"}], 8
)
p = u.get("prompt_tokens", 0)
out["cold_prefill"] = {
    "sec": round(sec, 2),
    "prompt": p,
    "tok_s": round(p / max(sec, 1e-9)),
}
...
for _ in range(3):
    _, ttft, _ = call(
        [{"role": "user", "content": "Reply with the single word: ok"}], 16, stream=True
    )
    ts.append(ttft)
...
# 3) decode tok/s (bs=1)
sec, ttft, u = call(
    [{"role": "user", "content": "Count from 1 to 400, comma separated."}],
    1200,
    stream=True,
)
...
print(
    f"3) decode bs1: {c} tok / {sec-(ttft or 0):.1f}s = {c/max(sec-(ttft or 0),1e-9):6.1f} tok/s",
    flush=True,
)
# 4) 長文prefill中の短文TTFT
res = {}
